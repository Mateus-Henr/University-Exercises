...
name, grade, newGrade = input("Name: "), float(input("Old grade: ")), float(input("New grade: "))

names, grades = [x.strip('\n') for x in namesFile.readlines()], []
for x in range(len(names)): grades.append([float(y) for y in gradesFile.readline().split()])
    ...
    if grade in grades[idx]:
        ...
        for x in range(len(grades)):
            for y in range(len(grades[x])):
                gradesFile.write(str(grades[x][y]) + (" " if y < len(grades[x]) - 1 else ""))
            if x < len(grades) - 1: 
                gradesFile.write("\n")
